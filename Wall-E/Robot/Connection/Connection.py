...
class Connection(Robot):
    ...
    def process(self, transferDirection, sensation):
        ...
        if sensation.getSensationType() == Sensation.SensationType.Item:
            ...
            if candidate_for_connect is not None and \
               len(candidate_for_connect.getConnections()) < Sensation.CONNECTIONS_MAX_CONNECTIONS and \
        ...
        if len(sensation.getConnections()) < Sensation.CONNECTIONS_MAX_CONNECTIONS:
            ...
            if candidate_to_connect is not None:
                ...
                if new_connection and len(candidate_to_connect.getConnections()) < Sensation.CONNECTIONS_MAX_CONNECTIONS:                
    ...
    #                    candidate_to_connect = Sensation.create(sensation=candidate_to_connect, memory=Sensation.Memory.LongTerm)
            ...
            for candidate_to_connect in candidates_to_connect:
                if candidate_to_connect is not sensation and len(candidate_to_connect.getConnections()) < Sensation.CONNECTIONS_MAX_CONNECTIONS:
...
#                         self.getParent().getAxon().put(transferDirection=Sensation.TransferDirection.Up, sensation=voiceSensation)
            ...
            self.log('99: process: sensation has reached connection max ' + time.ctime(sensation.getTime()) + ' ' + str(transferDirection) +  ' ' + sensation.toDebugStr())
