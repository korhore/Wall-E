...
class Hearing(Thread):
    ...
    Theory says, that man knows location of sound from timing. Sound must go longer way to that ear that is in longer way, so 
    if left ear is nearer, we hear sounf first in that ear. We implement this.
    
    Unfortunately Raspberry Pi with two microphones and threading python platform is not fast enough for this, I'm afraid.
    Soung goes 340 m/s and if I can give 20cm difference of microphones, his means that faster sound is only
    ...
    So we must do also something second best. We will also think that if only one microphone hears a sound,
            ...
            if not self.is_sound[other]:
                ...
                if sound.get_start_time() - other_sound.get_stop_time() < Hearing.SOUND_LIMIT:
                    ...
                    if other_sound.get_volume_level() > 0.0:
                        print "Sound from " + Hearing.ear_names[id] + " direction by volume level " + str(sound.get_volume_level()/other_sound.get_volume_level())
                    else:
                ...
                if other_sound.get_start_time() - sound.get_start_time() < Hearing.SOUND_LIMIT:
                    print "Sound from " + Hearing.ear_names[id] + " direction by timing " + str(other_sound.get_start_time() - sound.get_start_time())
                else:
                    if other_sound.get_volume_level() > 0.0:
                        print "Sound from " + Hearing.ear_names[id] + " direction by volume level " + str(sound.get_volume_level()/other_sound.get_volume_level())
                    else:
