...
class Robot(Thread):
    ...
    def __init__(self,
        ...
        self.setName(self.config.getName())
        self.setLocation(self.config.getLocation())
        self.log(logLevel=Robot.LogLevel.Normal, logStr="init robot name: " + self.getName() + " location: " + self.getLocation() + " kind: " + self.getKind() + " instanceType: " + self.getInstanceType() + " capabilities: " + self.capabilities.toDebugString())
        ...
        for subInstanceName in self.config.getSubInstanceNames():
            ...
            robot = self.loadSubRobotOneDir(subInstanceName=subInstanceName, level=self.level)
...
#             Robot.mainRobotInstance = self          # singleton instance
    ...
    def loadSubRobotOneDir(self, subInstanceName, level):
        robot = None
        try:
            module = subInstanceName
            imported_module = importlib.import_module("Robot."+module)
            self.log(logLevel=Robot.LogLevel.Detailed, logStr='init ' + subInstanceName)
            robot = getattr(imported_module, subInstanceName)(parent=self,
                                                              memory=self.getMemory(),  # use same memory than self
                                                              instanceName=subInstanceName,
        ...
        for robot in self.getSubInstances():
            if robot.getCapabilities().hasCapability(robotType, memoryType, sensationType, locations) or \
               robot.hasSubCapability(robotType, memoryType, sensationType, locations):
                self.log(logLevel=Robot.LogLevel.Verbose, logStr='hasSubCapability subInstance ' + robot.getName() + ' at ' + robot.getLocation() + ' has robotType ' + str(robotType) + ' memoryType ' + str(memoryType) + ' sensationType ' + str(sensationType) + ' True')      
                return True
...
#     
    ...
    def log(self, logStr, logLevel=LogLevel.Normal):
         if logLevel <= self.getLogLevel():
             print(self.getName() + ":" + str(self.config.level) + ":" + Sensation.Modes[self.mode] + ":" + self.getLocation() + ": " + logStr)
                ...
                for robot in robots:
                    if robot.getInstanceType() == Sensation.InstanceType.Remote:
                        ...
                        if sensation.isReceivedFrom(robot.getHost()) or \
                            ...
                            self.log(logLevel=Robot.LogLevel.Verbose, logStr='Remote robot ' + robot.getName() + 'has capability for this, but sensation comes from it self. Don\'t recycle it')
                        else:
                            self.log(logLevel=Robot.LogLevel.Verbose, logStr='Remote robot ' + robot.getName() + ' has capability for this, robot.getAxon().put(robot=self, transferDirection=Sensation.TransferDirection.Down, sensation=sensation)')
                        ...
                        self.log(logLevel=Robot.LogLevel.Verbose, logStr='Local robot ' + robot.getName() + ' has capability for this, robot.getAxon().put(robot=self, transferDirection=Sensation.TransferDirection.Down, sensation=sensation)')
                ...
                if robot.getInstanceType() == Sensation.InstanceType.Remote:
                    ...
                    if sensation.isReceivedFrom(robot.getHost()) or \
                       sensation.isReceivedFrom(robot.getSocketServer().getHost()):
                        ...
                        self.log(logLevel=Robot.LogLevel.Detailed, logStr='Remote robot ' + robot.getName() + ' has capability for this, robot.getAxon().put(robot=self, sensation)')
                    ...
                    self.log(logLevel=Robot.LogLevel.Verbose, logStr='Local robot ' + robot.getName() + ' has capability for this, robot.getAxon().put(robot=self, sensation)')
                    robot.getAxon().put(robot=self, transferDirection=transferDirection, sensation=sensation, detach=False) # keep ownerdhip untill sent to all sub Robots
                    needToDetach=False
            if needToDetach:
                sensation.detach(robot=self) # detach if no subRobot found
...
#             self.imageind = 0
        ...
        for i in range(Identity.VOICES_PER_CONVERSATION):          
            ...
            voiceSensation = self.createSensation(associations=[], sensation=self.getParent().voiceSensations[voiceind], memoryType = Sensation.MemoryType.Sensory, robotType = Sensation.RobotType.Sense,
                                                  locations='') # valid everywhere
            self.getParent().getAxon().put(robot=self, transferDirection=Sensation.TransferDirection.Up, sensation=voiceSensation) # or self.process
    ...
    def run(self):
        ...
        try:
            ...
            if self.running:
                ...
                sensation=self.getMainRobot().createSensation(associations=[], robotType=Sensation.RobotType.Muscle, sensationType = Sensation.SensationType.Capability, capabilities=capabilities)
